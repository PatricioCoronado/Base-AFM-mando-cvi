...
if(event==EVENT_LEFT_CLICK)
{
	...
	EsperaHandle = LoadPanel (ConfigHandle, "PanelPuertoCOM.uir", ESPERA);
...
{   
	...
	{   // Si el panel está recien abierto...
		...
		if(FlagPuertoAbierto)SetCtrlAttribute (ConfigHandle, CONFIGURAP_APLICA_TR, ATTR_DIMMED,0);
		else SetCtrlAttribute (ConfigHandle, CONFIGURAP_APLICA_TR, ATTR_DIMMED,1);
		...
		void *callbackData, int eventData1, int eventData2)
{
	if(event== EVENT_RIGHT_DOUBLE_CLICK)
	{
		...
		{
			if(PuertoCOM!=-1 && abrir_puerto(PuertoCOM, NombrePuertoCOM, VelocidadPuerto, Paridad,
		...
		int eventData1, int eventData2)
{
	...
	switch (event)
	{
		...
		case EVENT_GOT_FOCUS:
			...
			if(FlagPuertoAbierto) 
			{				
				SetCtrlAttribute (MensajesHandle, MENSAJES_LEDCOM, ATTR_DIMMED,0);
				SetMenuBarAttribute (menuHandle, MENUSCPI_COM, ATTR_DIMMED, 0);        
				SetMenuBarAttribute (menuHandle, MENUSCPI_MENUERROR, ATTR_DIMMED, 0);  
				SetMenuBarAttribute (menuHandle, MENUSCPI_MENUVERSION, ATTR_DIMMED,0);
				SetMenuBarAttribute (menuHandle, MENUSCPI_MENUCLS, ATTR_DIMMED, 0);
				SetCtrlAttribute (MensajesHandle, MENSAJES_COMANDO, ATTR_DIMMED, 0);
			}
			else
			{	
				SetCtrlAttribute (MensajesHandle, MENSAJES_LEDCOM, ATTR_DIMMED,1);
				SetMenuBarAttribute (menuHandle, MENUSCPI_COM, ATTR_DIMMED, 1);        
				SetMenuBarAttribute (menuHandle, MENUSCPI_MENUERROR, ATTR_DIMMED, 1);  
				SetMenuBarAttribute (menuHandle, MENUSCPI_MENUVERSION, ATTR_DIMMED, 1);
				SetMenuBarAttribute (menuHandle, MENUSCPI_MENUCLS, ATTR_DIMMED, 1);
				SetCtrlAttribute (MensajesHandle, MENSAJES_COMANDO, ATTR_DIMMED, 1);
			}
		break;
	}
	return 0;
...
#define PANEL_PADRE 0	//0 para que sea top level 1 para que no lo sea
	if ((ConfigHandle = LoadPanel (PANEL_PADRE, "PanelPuertoCOM.uir", CONFIGURAP)) < 0)
	return -1;
	else 
	{
		//cambia_de_modo_controles_de_mensajes();
...
{                                                                                                                                  
	...
	if (!FlagPuertoAbierto) {MessagePopup ("Mensaje RS232", "No hay ningún puerto abierto");return 0;}
	...
	else // RECEPION
	{
		...
		if(BytesEnBuffer==0){MessagePopup ("Mensaje RS232", "No hay datos en el buffer");return 0;}
		if(BytesEnBuffer<0){display_error_RS232();return 0;}// Si la lectura da un error lo avisa y sale   		
		if(BytesEnBuffer > STRLARGO){ErrorRS232=ERROR_DATOS_EXCESIVOS;display_error_RS232();return 0;}
		{   
			BytesLeidos=ComRd (PuertoCOM, CadenaLeida, BytesEnBuffer);
			if(BytesLeidos <0){ErrorRS232=BytesLeidos; display_error_RS232(); return 0;}
		}
...
void CVICALLBACK autor (int menuBar, int menuItem, void *callbackData,
		int panel)                                                    
{                                                                     
	 MessagePopup ("PCOM",MENSAJE_AUTOR);
}                                                                     
/******************************************************************************************************/
